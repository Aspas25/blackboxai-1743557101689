class AnalyticsSystem {
    ...
    async loadData() {
        try {
            ...
            utils.showFeedback('Erro ao carregar dados', 'error');
        }
    }
    ...
    updateMetrics() {
        ...
        document.getElementById('total-metrics').innerHTML = `
            <p class="text-3xl font-bold text-gray-900">${utils.formatCurrency(totalMetrics.value)}</p>
            <p class="text-sm text-gray-600">${totalMetrics.count} oportunidades</p>
        `;

        document.getElementById('average-metrics').innerHTML = `
            <p class="text-3xl font-bold text-blue-600">${utils.formatCurrency(averageValue)}</p>
    ...
    createEvolutionChart(groupBy) {
        ...
        this.charts.evolution = new Chart(ctx, {
            type: 'line',
            data: {
                labels: this.currentData.map(d => utils.formatDate(d.date)),
                datasets: datasets
            },
            options: {
                responsive: true,
            ...
            return `
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        ${utils.formatCurrency(group.prospeccao.value)}
                        <span class="text-xs text-gray-400">(${group.prospeccao.count})</span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        ${utils.formatCurrency(group.negociacao.value)}
                        <span class="text-xs text-gray-400">(${group.negociacao.count})</span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        ${utils.formatCurrency(group.fechamento.value)}
